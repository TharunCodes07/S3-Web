...
from code2 import (
    ...
    list_converter, db, search_result, chat, chat2
)
import time
import os
...
async def startup_event():
    print('Loading')
    global collection
    collection = create_collection(r'D:\Coding\React\Project UI\LearnScope\VerdictIQ\server\Respira\authors_book.pdf','Laws')
...
async def get_summary(request:QueryRequest):
    ...
    print(predictions)
    time.sleep(5)
    response = chat2.predict(prediction=predictions)
    return {"response":response['response']}
